...
impl HostDataSink {
    pub fn new() -> Self { Self { sent: Vec::new(), fail_next: false } }

    pub fn sent_count(&self) -> usize              { self.sent.len() }
    pub fn last_sent(&self) -> Option<&PayloadSample> { self.sent.last() }
...
impl DataSink for HostDataSink {
    fn send(&mut self, sample: &PayloadSample) -> anyhow::Result<()> {
        match std::mem::replace(&mut self.fail_next, false) {
            ...
            false => { self.sent.push(PayloadSample { ..*sample }); Ok(()) }
        }
    }
}
...
mod tests {
    ...
    fn sample(d: u32) -> PayloadSample {
        PayloadSample { pulse_delta: d, time_delta_ms: 1_000, accumulative_pulse: d }
    }
    ...
    fn fail_next_recovers_on_next_call() {
        let mut sink = HostDataSink { sent: vec![], fail_next: true };
